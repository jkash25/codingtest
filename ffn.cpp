...
// Optional: keep verify_results for small batches using float32 reference
bool verify_results(float* h_out, float* h_x, float* h_Wu, float* h_Wv, float* h_Wo, int B, int hidden, int inter) {
    printf("Verifying Sample of Batch %d... ", B);
    float epsilon = 1e-3f; 
...
int main() {
    ...
    std::cout << "GEGLU FFN BENCHMARK (FP16)" << std::endl;
    ...
    for (int B : batch_sizes) {
        ...
        __half *x, *out;
        CUDA_CHECK(cudaMalloc(&x,  B * HIDDEN * sizeof(__half)));
        CUDA_CHECK(cudaMalloc(&out,B * HIDDEN * sizeof(__half)));

        std::vector<float> h_x(B * HIDDEN);
        initialize(h_x.data(), h_x.size());

        // Convert to FP16
        std::vector<__half> h_x_half(B * HIDDEN);
        for (int i = 0; i < B * HIDDEN; ++i) {
            h_x_half[i] = __float2half(h_x[i]);
        }

        CUDA_CHECK(cudaMemcpy(x, h_x_half.data(), h_x_half.size() * sizeof(__half), cudaMemcpyHostToDevice));
        ...
        // Optional: verification for small batches
        // if (B <= 8) { ... same as before ... }
